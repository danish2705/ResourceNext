...
import {
  DashboardService,
  type SavedDashboard,
} from "@/components/DashboardService";
...
const MOCK = {
  ...
  overallocated: [
    { name: "John Smith", pct: 140 },
    { name: "Sarah Wilson", pct: 125 },
    { name: "Karthik Rao", pct: 115 },
  ],
  topUnfilled: [
  ...
  resourceAvail: [
    ...
    { skill: "Azure", count: 12 },
    { skill: "SAP", count: 6 },
    { skill: "Oracle", count: 5 },
  ],
  ...
  myAssignments: [
    {
      ...
      role: "Developer",
      alloc: 60,
      period: "01 Mar – 30 Jun",
    },
    {
      project: "Phoenix",
      role: "Data Analyst",
      alloc: 20,
      period: "16 Apr – 15 Jun",
    },
  ],
  myTasks: {
    todo: [
  ...
  notifications: [
    ...
    {
      msg: "Project Allocation Updated",
      time: "1 day ago",
      color: T.amber,
      dot: T.amber,
    },
    ...
    >
      ...
      >
        {deltaUp ? (
          ...
          <ArrowDownRight size={10} color={T.red} />
        )}
...
function QuickActionBtn({
  ...
  return (
    <button
      ...
      onMouseLeave={(e) =>
        (e.currentTarget.style.background = bg || `${color}15`)
      }
    >
      <Icon size={12} />
      {label}
...
function CustomizeSidebar({
  ...
  const tabItems =
    tab === "widgets" ? widgets : tab === "kpi" ? kpiCards : filters;
  const tabDragRef =
    tab === "widgets"
      ? widgetDragRef
      : tab === "kpi"
    ...
    >
        ...
        {[
          ...
          <button
            ...
            style={{
              ...
              border: "none",
              borderBottom:
                tab === t.id ? `2px solid ${T.blue}` : "2px solid transparent",
      ...
      >
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
          {tabItems.map((item, i) => {
            const isOver =
              dragGroup === tab && dragOver === i && dragging !== i;
                ...
                style={{
                  ...
                  background: isOver
                    ...
                    : item.checked
                      ? T.dragChecked
                      : T.dragUnchecked,
                  border: `1px solid ${isOver ? T.blue : item.checked ? T.dragCheckedBorder : T.border}`,
                  opacity: isDragging ? 0.4 : 1,
                  cursor: isMandatory ? "default" : "grab",
                  userSelect: "none",
                }}
              >
                <label
      ...
      <div style={{ padding: "12px 14px", borderTop: `1px solid ${T.border}` }}>
        <button
          ...
          style={{
            ...
            boxShadow: allHidden ? "none" : "0 2px 10px rgba(37,99,235,0.3)",
        ...
        {allHidden && (
          ...
          >
            Enable at least one widget to save.
          </p>
        )}
      </div>
    </div>
...
const PMO_WIDGET_DEFS: WidgetConfig[] = [
  ...
  {
    id: "pmo_staffing",
    label: "Project Staffing Overview",
    checked: true,
    row: 2,
  },
  {
    id: "pmo_forecast",
    label: "Upcoming Demand Forecast",
    checked: true,
    row: 2,
  },
...
const RM_KPI_DEFS = [
  ...
  },
  {
    ...
    iconBg: C.redSoft,
    iconColor: C.red,
    label: "Upcoming Roll-Offs",
    value: "7",
    vsLabel: "Next 30 days",
    delta: "2%",
    deltaUp: false,
    valueColor: T.red,
  },
...
const RM_WIDGET_DEFS: WidgetConfig[] = [
  {
    id: "rm_heatmap",
    label: "Team Utilization Heatmap",
    checked: true,
    row: 1,
  },
  {
  ...
  switch (role) {
    case "super_admin":
      return {
        persona: "super_admin",
        kpiDefs: SA_KPI_DEFS,
        widgetDefs: SA_WIDGET_DEFS,
      };
    case "pmo":
      return {
        persona: "pmo",
        kpiDefs: PMO_KPI_DEFS,
        widgetDefs: PMO_WIDGET_DEFS,
      };
    case "resource_manager":
      return {
        persona: "resource_manager",
        kpiDefs: RM_KPI_DEFS,
        widgetDefs: RM_WIDGET_DEFS,
      };
    case "resource":
      return {
        persona: "resource",
        kpiDefs: MY_KPI_DEFS,
        widgetDefs: MY_WIDGET_DEFS,
      };
    default:
      return {
        persona: "resource",
  ...
  switch (role) {
    case "super_admin":
      return {
        ...
        subtitle: "Enterprise-wide visibility and governance.",
      };
    case "pmo":
      return {
        title: "PMO Dashboard",
        subtitle: "Workforce planning overview and demand fulfillment status.",
      };
    case "resource_manager":
      return {
        title: "Resource Manager Dashboard",
        subtitle:
          "Manage your team's allocation, utilization and resource availability.",
      };
    case "resource":
      return {
        title: "Resource Dashboard",
        subtitle: "Your personal work summary and tasks.",
      };
    default:
    ...
    case "sa_overview":
      return (
        <CardShell title="System Overview">
          {[
            ["Business Units", "12", T.blue],
            ["Domains", "21", T.green],
            ["Practices", "28", T.purple],
            ["Locations", "15", T.orange],
            ["Active Workflows", "9", T.teal],
          ].map(([l, v, c]) => (
            <div
              key={l}
              style={{
        ...
        <CardShell title="Utilization Trend (Last 12 Months)">
          <ResponsiveContainer width="100%" height={180}>
            ...
            >
              ...
              <defs>
                <linearGradient id="saUG" x1="0" y1="0" x2="0" y2="1">
          ...
          <div style={{ textAlign: "right", fontSize: 10, color: T.textFaint }}>
            May 76% <span style={{ color: T.blue, fontWeight: 700 }}>▲</span>
          </div>
        </CardShell>
      );
    case "sa_alerts":
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {[
              ...
              >
                ...
                <StatusBadge color={s.color} bg={`${s.color}15`}>
                  {s.status}
                </StatusBadge>
              </div>
            ))}
        ...
        <CardShell title="User Management Summary">
          ...
          >
            {MOCK.userMgmt.map((u, i) => (
              ...
              >
                <div style={{ fontSize: 18, fontWeight: 800, color: u.color }}>
        ...
        <CardShell title="Top Unfilled Demands" subtitle="By role & gap">
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {MOCK.topUnfilled.map((r, i) => (
              ...
              >
                <span
                  style={{ fontSize: 10.5, color: T.textSec, minWidth: 100 }}
                >
                  {r.role}
                </span>
                <MiniBar
                ...
                {MOCK.projectStaffing.map((r, i) => (
                  ...
                  >
                    ...
                    <td style={{ padding: "5px 6px", textAlign: "right" }}>
                      <span
                        style={{
                          color: parseInt(r.util) >= 90 ? T.green : T.amber,
                          fontWeight: 700,
                        }}
                      >
                        {r.util}
        ...
        <CardShell title="Upcoming Demand Forecast" subtitle="Next 6 months">
          <ResponsiveContainer width="100%" height={160}>
            ...
            >
              ...
              <Tooltip content={<CustomTooltip />} />
              <defs>
                <linearGradient id="pmoFG" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor={T.teal} stopOpacity={0.25} />
                  <stop offset="100%" stopColor={T.teal} stopOpacity={0.01} />
                </linearGradient>
              </defs>
              <Area
                type="monotone"
        ...
        <CardShell title="Team Utilization Heatmap">
          ...
          {MOCK.teamHeatmap.map((r, i) => {
            ...
            return (
              ...
              >
                <span style={{ fontSize: 10, color: T.textSec, minWidth: 88 }}>
                  {r.resource}
                </span>
                <div
                  style={{
                ...
                >
                  <div
                    style={{
                      width: `${Math.min((r.util / 130) * 100, 100)}%`,
                      height: "100%",
                      background: c.bar,
                      borderRadius: 4,
                    }}
        ...
        <CardShell title="Resource Availability" subtitle="Next 90 days">
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {MOCK.resourceAvail.map((r, i) => (
              ...
              >
                <span
                  style={{ fontSize: 10.5, color: T.textSec, minWidth: 92 }}
                >
                  {r.skill}
                </span>
        ...
        <CardShell title="Upcoming Roll-Offs" subtitle="Next 30 days">
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {MOCK.rollOffs.map((r, i) => (
              ...
              >
                ...
                <span style={{ color: T.red, fontWeight: 700, fontSize: 10.5 }}>
                  {r.date}
                </span>
              </div>
            ))}
          ...
          >
            <span style={{ color: T.orange, fontWeight: 600 }}>
              ⚠ 4 more roll-offs in August
            </span>
          </div>
        </CardShell>
      );
    case "rm_requests_table":
      return (
              ...
              <tbody>
                {MOCK.assignmentRequests.map((r, i) => (
                  ...
                  >
                    ...
                    <td style={{ padding: "5px 6px", color: T.textSec }}>
                      {r.project}
                    </td>
                    <td style={{ padding: "5px 6px", color: T.text }}>
                      {r.role}
                    </td>
                    <td style={{ padding: "5px 6px", color: T.textMuted }}>
                      {r.by}
                    </td>
                    <td style={{ padding: "5px 6px", color: T.textFaint }}>
                      {r.date}
                    </td>
        ...
        <CardShell title="Skill Distribution">
          <ResponsiveContainer width="100%" height={180}>
            <BarChart
              ...
              layout="vertical"
              barSize={9}
              margin={{ top: 4, right: 20, bottom: 4, left: 50 }}
            >
              <CartesianGrid
                strokeDasharray="2 2"
                stroke={T.borderLight}
    ...
    case "rm_bench":
      return (
        <CardShell
          ...
          subtitle="Available for immediate allocation"
        >
          <div style={{ overflowX: "auto" }}>
            <table
              style={{
            ...
            >
              <thead>
                <tr style={{ borderBottom: `1px solid ${T.border}` }}>
                  {["Resource", "Skill", "Experience", "Availability"].map(
                    (h) => (
                      <th
                        key={h}
                        style={{
              ...
              <tbody>
                {MOCK.bench.map((b, i) => (
                  ...
                  >
                    ...
                    <td style={{ padding: "5px 6px" }}>
                      <StatusBadge color={T.blue} bg={`${T.blue}15`}>
                        {b.skill}
                      </StatusBadge>
                    </td>
                    <td style={{ padding: "5px 6px", color: T.textSec }}>
                    ...
                    <td style={{ padding: "5px 6px" }}>
                      <StatusBadge
                        color={b.avail === "Immediate" ? T.green : T.amber}
                        bg={`${b.avail === "Immediate" ? T.green : T.amber}15`}
                      >
                        {b.avail}
                      </StatusBadge>
                    </td>
        ...
        <CardShell title="My Tasks">
          ...
          >
            {[
              ...
              {
                label: "In Progress",
                tasks: MOCK.myTasks.inprogress,
                color: T.blue,
              },
              {
                label: "Awaiting Approval",
                tasks: MOCK.myTasks.awaiting,
                color: T.amber,
              },
              {
                label: "Completed",
                tasks: MOCK.myTasks.completed,
                color: T.green,
              },
            ].map((col) => (
              <div
          ...
          >
            <div
              style={{
                ...
                height: 110,
                borderRadius: "50%",
                border: `10px solid ${T.blue}`,
                display: "flex",
                flexDirection: "column",
                alignItems: "center",
                justifyContent: "center",
                boxShadow: `0 0 0 3px ${T.blue}20`,
              }}
            >
              <span
                style={{
            ...
            {MOCK.notifications.map((n, i) => (
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div
                    style={{ fontSize: 10, color: T.textFaint, marginTop: 1 }}
                  >
                    {n.time}
                  </div>
                </div>
          ...
          >
            {[
              ...
              >
                ...
                <div
                  style={{ fontSize: 9.5, color: T.textFaint, marginTop: 2 }}
                >
                  {l}
                </div>
              </div>
...
function DashboardContent({ role, widgets, kpiCards }: DashboardContentProps) {
  ...
  const visibleKpi = kpiCards
    .filter((k) => k.checked)
    .map((k) => kpiDefMap.get(k.id))
    .filter(Boolean);
  ...
  const rowLabels: Record<number, Record<string, string>> = {
    super_admin: {
      ...
      2: "Resource & System Insights",
      3: "Team & Actions",
    },
    pmo: {
      1: "Demand Pipeline & Alerts",
      2: "Demand Analysis & Staffing",
      3: "Forecast & Actions",
    },
    resource_manager: {
      1: "Team Overview",
  ...
  function renderWidget(id: string) {
    switch (role) {
      case "super_admin":
        return renderSAWidget(id, axisProps, router.push.bind(router));
      case "pmo":
        return renderPMOWidget(id, axisProps, router.push.bind(router));
      case "resource_manager":
        return renderRMWidget(id, axisProps, router.push.bind(router));
      case "resource":
        return renderMyWidget(id, router.push.bind(router));
      default:
        return null;
    }
  }
    ...
    return (
      ...
      >
        ...
        <div style={{ fontSize: 12, color: T.textFaint }}>
          Open Customize Dashboard to enable widgets.
        </div>
      </div>
    );
  }
    ...
    >
      {visibleKpi.length > 0 && (
        <>
          ...
          <div
            style={{
              display: "grid",
              gridTemplateColumns: `repeat(${Math.min(visibleKpi.length, 6)}, minmax(0,1fr))`,
              gap: 10,
              marginBottom: 8,
            }}
          >
            {visibleKpi.map((k) => (
              <KpiCard key={k!.id} {...(k as any)} />
            ))}
          </div>
        </>
      )}
      {rowMap.map(([rowNum, rowWidgets]) => (
        <div key={rowNum} style={{ marginBottom: 4 }}>
          <DashSectionLabel>
            {(rowLabels as any)[role]?.[rowNum] ?? `Section ${rowNum}`}
          </DashSectionLabel>
          <div
            style={{
              display: "grid",
              gridTemplateColumns: `repeat(${rowWidgets.length}, minmax(0,1fr))`,
              gap: 12,
              marginBottom: 4,
            }}
          >
...
function RoleDashboard({ role }: { role: Role }) {
  ...
  const [activeViewName, setActiveViewName] =
    useState<string>(DEFAULT_VIEW_NAME);
  ...
  function handleSelectView(id: string) {
    ...
    const mergedWidgets = widgetDefs
      .map((w, i) => {
        const saved = dash.widgetConfig.find((s) => s.id === w.id);
        return saved
          ? { ...w, checked: saved.checked, order: saved.order ?? i }
          : { ...w, order: i };
      })
    ...
    const mergedFilters = DEFAULT_FILTERS.map((f, i) => {
      ...
      if (saved)
        return {
          ...f,
          checked: f.mandatory ? true : saved.checked,
          order: saved.order ?? i,
        };
      return { ...f, order: i };
  ...
  const handleDialogSaved = (dashId: string, dashName: string) => {
    ...
    router.push(
      `/my-dashboard?fromSave=1&dashboardId=${dashId}&dashboardName=${encodeURIComponent(dashName)}`,
    );
  };
